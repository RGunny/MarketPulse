 ...
 * 종목 정보 도메인 엔티티
 * 
 * 주식 종목의 정적 메타데이터를 관리하는 핵심 도메인 모델
 * 종목코드, 종목명, 시장구분, 업종 등 변경 빈도가 낮은 마스터 데이터
 * 
 * 주요 특징:
 *   불변 객체(Immutable)로 설계되어 스레드 안전성 보장
 *   종목코드(symbol)를 unique 인덱스로 사용
 *   시장구분과 종목코드를 복합 인덱스로 구성
 *   거래 가능 여부, ETF/SPAC 구분 등 종목 분류 정보 포함
 *
 * @see StockPrice 시계열 가격 데이터는 별도 엔티티로 관리
 */
@Getter
@Builder
...
public class Stock {
    ...
    /**
     * 주요 종목 여부 (KOSPI200, KOSDAQ150 구성 종목 등)
     */
    public boolean isMajorStock() {
        // TODO: 추후 KOSPI200, KOSDAQ150 구성 종목 정보 추가 시 구현
